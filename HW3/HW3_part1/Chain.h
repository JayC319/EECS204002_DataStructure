...
void Chain<T>::deleteBack() {
    ...
    if(!first->link) {
        ...
        ChainNode<T>* deleteNode = cur->link;
        cur->link = NULL;
        delete deleteNode;
    }
...
T Chain<T>::Get(int k) {
    if(!first)
        cerr << "Given index unattatainable";
 ...
 */
template <class T>
void Chain<T>::insertBeforeK(int k, T& newData) {
    ChainNode<T>* cur = this->first;
    ChainNode<T>* newNode = new ChainNode<T>;
